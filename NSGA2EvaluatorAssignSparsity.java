...
public class NSGA2EvaluatorAssignSparsity extends NSGA2Evaluator{
	public void assignSparsity(Individual[] front) {
		...
		for (int i = 0; i < numObjectives; i++) {
			...
			//Gaofeng Shi 2022.08.10 COMP589 ++++++++++++++++++++++++++++++++++++++++++
			double alpha = NSGA2Evaluator.alphaMatrix[0]; // get the alpha dominance.
...
//			    System.out.println("original max: " + f_j.maxObjective[o] );
				...
				if (max ==  min)
            	{
					...
					//Gaofeng Shi, test the dominance criteria. the result is not good. so don't use it +++++++++++++++++++++++++++++
//					if (o == 0) {
//						f_j.sparsity += (f_jplus1.getObjective(o) - f_jminus1.getObjective(o)) / (max - min);
//					}
//					else{
//						f_j.sparsity += (f_jplus1.getObjective(o) - f_jminus1.getObjective(o)) / (max - min)
//						+ alpha * ((f_jplus1.getObjective(0) - f_jminus1.getObjective(0))
//						/(((NSGA2MultiObjectiveFitness) front[front.length - 1].fitness).getObjective(0)
//								-((NSGA2MultiObjectiveFitness) front[0].fitness).getObjective(0))); // objective1 will add the alpha dominance.
//					}
				}
